...
let NavBar: FC<Props> = ({ }) => {
    ...
    return (
        <div className='flex flex-col md:flex-row md:justify-between  gap-2  items-center py-5 px-[5%] bg-background sticky top-0 z-[4]'>
            ...
            <div className='flex flex-row items-center py-2 px-4 gap-3 w-[100%] md:w-[35%]  md:ml-[5%] bg-light-500 rounded-[5px] shadow-md' onClick={() => {
                dispatch(setSearch(true));
            }}>
                <i className="uil uil-search text-xs text-500 cursor-pointer"></i>
                <div className='bg-[transparent] w-[100%] outline-none text-text-500 text-2xs font-r placeholder:font-r placeholder:text-2xs placeholder:text-text-500 cursor-pointer'>Search in Store</div>
